 ...
 */
class SketchBoard extends PureComponent {
  static propTypes = {
  ...
  _initTools = fabricCanvas => {
    ...
    this._tools[Tool.Pencil] = new Pencil(fabricCanvas,this.props.isActiveEraser);
    ...
    this._tools[Tool.Text] = new Text(fabricCanvas,this.props.fontSize);
   ...
   /**  
   * Update Canvas Objects in Redux
   */

  updatecanavsDataRedux = () =>{
     if(this._fc!==undefined){
      var json = this._fc.toJSON();
      var historyJson= [] //this._history.toJSON();
      let dataObj = {
          _fc:json,
          _history:historyJson,
          _hasActive:true 
      }
      // this.props.setWhiteBoardData(dataObj)
    }
  }
  /**
  ...
  undo = () => {
    ...
    if(obj!==undefined && obj?.type !==undefined && obj.text!==undefined && obj.text===''){
      let [obj2, prevState, currState] = history.getCurrent();
      history.undo(true);
      if (obj2.__removed) {
        this.setState({ action: false }, () => {
          this._fc.add(obj2);
          obj2.__version -= 1;
          obj2.__removed = false;
        });
      } else if (obj2.__version <= 1) {
        this._fc.remove(obj2);
      } else {
        obj2.__version -= 1;
        obj2.setOptions(JSON.parse(prevState));
        obj2.setCoords();
        this._fc.renderAll();
      }
      if (this.props.onChange) {
  ...
  changeSelectedFillColor = color => {
    let canvas = this._fc;
    let activeObj = canvas.getActiveObject();
    if (activeObj && activeObj?.type!==undefined && activeObj?.type=== 'i-text') {
   ...
   /**
   * Set object strokeWidth
   */

  changeSelectedStorkeWidth= color => {
    let canvas = this._fc;
    let activeObj = canvas.getActiveObject();
    if (activeObj && activeObj?.type!==undefined && activeObj?.type=== 'i-text') {
      activeObj.set({ fontSize: this.props.fontSize });
      canvas.requestRenderAll();
      canvas.renderAll();
      canvas.fire("object:modified", { target: activeObj });
      ...
      if(activeObj){
        activeObj.set({ strokeWidth: this.props.lineWidth });
        canvas.renderAll();
        canvas.fire("object:modified", { target: activeObj });
      }
    }
  ...
  componentDidMount = () => {
    ...
    if (!this._fc) {
      let canvas = "";
      if (
        this.props.whiteBoardData !== undefined &&
        this.props.whiteBoardData._fc !== undefined &&
        this.props.whiteBoardData._history !== undefined &&
        this.props.whiteBoardData._fc !== null &&
        this.props.whiteBoardData._history !== null
      ) {
        let canvasCopy = new fabric.Canvas(this._canvas);
        canvasCopy.loadFromJSON(this?.props?.whiteBoardData?._fc);
        canvas = this._fc = canvasCopy;
        // [1, 2, 3, 4, 5, 6, 7, 8].forEach(() => {
        //   this.zoom(0.8);
        // });
      } else {
        canvas = this._fc = isPreview
  ...
  componentDidUpdate = (prevProps, prevState) => {
    ...
    // console.log("print All Canvas Data Update", this._history);
    // let dataObj = {
    //       _fc:this._fc,
    //       _history:this._history,
    //       _hasActive:true
    // }
    // if(!this.props.WhiteBoardEnable){
    //   this.props.dispatch(setWhiteBoardData(dataObj));
    // }
    // this.props.setWhiteBoardData(dataObj);
    // this.props.updateWhiteBoardData({
    //     _fc:this._fc,
    //     _history:this._history,
    //     _hasActive:true
    // })
      ...
      >
        <div style={{ display: "flex" }}>
          <SideBar
            ...
            changeSelectedStrokeColor={this.changeSelectedStrokeColor}
            changeSelectedFillColor={this.changeSelectedFillColor}
            canRedo={this.canRedo}
            canvas={this._fc}
            setLecturePdfFilePath={this.props.setLecturePdfFilePath}
            ...
            WhiteBoardEnable={this.props.WhiteBoardEnable}
            _history={this._history}
            changeSelectedStorkeWidth={this.changeSelectedStorkeWidth}
            lineWidth={ this.props.lineWidth }
            setSelected={this.setSelected}
            sizeColorTool={this.props.sizeColorTool}
            selectedType={this.props.tool}
            deselectActiveObject={this.deselectActiveObject}
            clearWhiteBoardData={this.props.clearWhiteBoardData}
            setClearWhiteBoardData={this.props.setClearWhiteBoardData}
            downloadFunctionCall={this?.props?.downloadFunctionCall}
            setDownloadFunctionCall={this?.props?.setDownloadFunctionCall}
            lecturePdfFilePath={this?.props?.lecturePdfFilePath}
            updatecanavsDataRedux={this.updatecanavsDataRedux}
            saveWhiteBoardData={this?.props?.saveWhiteBoardData}
            setSaveWhiteBoardData={this?.props?.setSaveWhiteBoardData}
       />
          <canvas style={{marginLeft: "2px"}}  id={uuid4()} ref={c => (this._canvas = c)}>
